...
import pandas as pd
from model import Talk, Outcome, connect_to_db, db
...
def load_talks():
    ...
    while i < 2550:
        ...
        published_date=talks.iloc[i,9]
        talk_name=talks.iloc[i,14]
        duration=talks.iloc[i,2]
        event=talks.iloc[i,3]
        languages=talks.iloc[5]
        talk_id=i
        i+=1
        speaker = Speaker(talk_id=talk_id,
                    speaker_name=speaker_name,
                    speaker_job=speaker_job, published_date=published_date, talk_name=talk_name,
                    duration = duration, event=event, langauges=languages)
...
def load_movies():
    ...
    for i, row in enumerate(open("speakers.txt")):
        row = row.rstrip()
